...
def _render_case_grid(db_path: Path, records_db_path: Path, rows: list[dict[str, object]], active_case_id: int | None, widths: list[float]) -> None:
    ...
    header_cells = "\n".join(
        ...
        if key in {"view", "export", "mail", "web", "edit", "del"}
...
def render(db_path: Path, records_db_path: Path) -> dict[str, object] | None:
    ...
    with size_col:
        # Avoid the warning "was created with a default value but also had its value set via the Session State API"
        # by ensuring session state is within bounds and NOT providing a 'value' parameter to the widget.
